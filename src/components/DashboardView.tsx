...
import { DashboardStatsData } from '../types'; // Corrected type import mapping

interface DashboardViewProps {
  stats: DashboardStatsData | null; // Bound to the standardized data layer name
  loading: boolean;
  onRefresh: () => void;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center space-x-3 self-start md:self-auto">
            <button
              onClick={onRefresh}
              disabled={loading}
              className="px-4 py-2.5 bg-slate-800/80 hover:bg-slate-700/80 border border-slate-700 text-slate-200 rounded-xl transition shadow-sm text-xs font-semibold flex items-center gap-1.5"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-3">
          ...
          <div className="flex items-baseline justify-between">
            ...
            <span className="text-xs font-semibold text-slate-400 font-mono">
              Unique: {stats ? stats.uniqueCustomersToday : 98}
            </span>
          </div>
          <p className="text-[11px] text-slate-400">Captured via OpenCV video pipeline components</p>
        </div>

        {/* Metric 2: Loyalty Match Rate */}
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
        ...
        {/* Metric 3: Customer Sentiment Index */}
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
        ...
        {/* Metric 4: Product Image Scans (Wired with crash-guard optional chaining) */}
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
          ...
          <div className="flex items-baseline justify-between">
            <span className="text-2xl font-extrabold text-slate-900">
              {stats?.categoryDistribution ? stats.categoryDistribution.reduce((a, b) => a + b.count, 0) : 141}
            </span>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Face encodings extracted and matched in real-time with instant loyalty profile analytics sync.
              </p>
            </div>
          ...
          <div className="space-y-3">
            {stats && stats.recentVisits && stats.recentVisits.length > 0 ? (
                  ...
                  <div className="flex items-center space-x-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${visit.status === 'VIP'
                          ? 'bg-amber-100 text-amber-800 border border-amber-300'
                          : 'bg-indigo-100 text-indigo-800 border border-indigo-200'
                          }`}>
                          {visit.status} ({visit.loyaltyTier || 'Silver'})
                        </span>
                      </div>
              ...
              <div className="p-8 text-center text-xs text-slate-400 font-mono">Awaiting stream telemetry metrics...</div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
            ...
            <div className="space-y-3">
              {stats?.categoryDistribution ? (
                ...
                <div className="text-xs text-slate-400 font-mono">Awaiting model distribution statistics...</div>
              )}
            </div>
          </div>

          {/* Module C4 Subsystem Telemetry Check Panel */}
          <div className="bg-slate-900 text-white rounded-2xl p-5 shadow-md border border-slate-800 space-y-3">
            <h4 className="text-xs font-extrabold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
              ...
              Submodule Pipeline Status (C1/C4)
            </h4>
            <div className="grid grid-cols-2 gap-2 text-[11px] font-mono">
              <div className="p-2 bg-slate-950/60 rounded-lg border border-slate-800/80 flex justify-between">
                ...
                <span className="text-emerald-400 font-bold">{stats?.systemStatus?.cvModule ?? 'Active'}</span>
              </div>
              <div className="p-2 bg-slate-950/60 rounded-lg border border-slate-800/80 flex justify-between">
                <span className="text-slate-400">NLP Engine:</span>
                <span className="text-emerald-400 font-bold">{stats?.systemStatus?.nlpModule ?? 'Active'}</span>
              </div>
              <div className="p-2 bg-slate-950/60 rounded-lg border border-slate-800/80 flex justify-between">
                <span className="text-slate-400">Chatbot Core:</span>
                <span className="text-emerald-400 font-bold">{stats?.systemStatus?.chatbotModule ?? 'Active'}</span>
              </div>
              <div className="p-2 bg-slate-950/60 rounded-lg border border-slate-800/80 flex justify-between">
                <span className="text-slate-400">Gateway:</span>
                <span className="text-indigo-400 font-bold">Operational</span>
              </div>
            </div>
          </div>
        </div>
